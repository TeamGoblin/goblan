<head>
	<meta charset="utf-8"/>
	<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1"/>
	<meta name="viewport" content="width=device-width, initial-scale=1"/>
	
	<title>GobLAN 2014 - December 19-21</title>

	<link rel="icon"
		  type="image/png"
		  href="/i/img/logo2.png" />
	
	<!-- JQueryUI Dark - http://jqueryui.com -->
	<link rel="stylesheet" href="/i/css/jquery-ui-1.10.3.custom.css" type="text/css" media="screen" />

	<!-- Twitter Bootstrap - http://twitter.github.com/bootstrap/ -->
	<link rel="stylesheet" href="/i/css/bootstrap.css" type="text/css" media="screen" />

	<!-- Flat UI - http://designmodo.github.io/Flat-UI/ -->
	<link rel="stylesheet" href="/i/css/flat-ui.css" type="text/css" media="screen" />

	<!-- Font-Awesome - http://fortawesome.github.com/Font-Awesome/ -->
	<link rel="stylesheet" href="/i/css/font-awesome.css" type="text/css" media="screen" />		

	<!-- Bootstrap 3 Grid - http://twitter.github.com/bootstrap -->
	<link rel="stylesheet" href="/i/css/bootstrap3.css" type="text/css" media="screen"/>
	
	<!-- Flipclock - http://flipclockjs.com -->
	<link rel="stylesheet" href="/i/css/flipclock.css" type="text/css" media="screen" />

	<!-- GobLAN - http://teamgoblin.com -->
	<link rel="stylesheet" href="/i/css/goblan.css" type="text/css" media="screen" />
	
	<!--[if lt IE 9]>
		<script src="//html5shim.googlecode.com/svn/trunk/html5.js"></script>
	<![endif]-->
</head>